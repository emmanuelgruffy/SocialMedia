...
router.put("/experience", [ auth, [
  ...
  router.delete('/experience/:exp_id', auth, async (req,res) => {
    try {
        ...
        //we map over our expereicnes and form an array of all the id's in the array.
        //then we get the index of the item ain the array that has the exp_id passed from the api.

        const newArray = profile.experience.filter( item => {
            return item.id !== req.params.exp_id
        });

        profile.experience = newArray;
        await profile.save();
        res.json(profile);
    } catch (error) {
        console.log(error.message);
...
router.put("/education", [ auth, [
    ...
    const newEdu = {
      ...
      try {
          ...
          profile.education.unshift(newEdu); //expereince is an array of experiences so we simply add to the last place of the array our new expereince
          await profile.save();
          res.json(profile);
...
router.delete('/education/:edu_id', auth, async (req, res) => {
    try {
        ...
        const newArray = profile.education.filter( item => {
            return item.id !== req.params.edu_id
        });

        profile.education = newArray;
        await profile.save();
        res.json(profile);
    } catch (error) {
        console.log(error.message);
