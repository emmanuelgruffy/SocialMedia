...
const EditProfile = ({ profile: { profile, loading }, createProfile, getCurrentProfile, history }) => { // history is a prop that will enable us to redirect in case needed.
    ...
    useEffect(() => {
        ...
        setFormData({ // we want to fill the profile form with the data we fetched, only in case page is done loading, or the field
            //is not empty of-course, hence the ternary operation you see below:
          ...
          //see the profile.social below - this is taken actually from the social field in the backend.
    ...
    const onSubmit = e => {
        e.preventDefault();
        createProfile(formData, history, true); //true = edit in the create or update profile action.
    }

    return (
        <Fragment>
            <h1 className="large text-primary profile-header">
                Create Your Profile
           </h1>
            <p className="lead">
